#!/bin/bash
# Check if no old data get lost. See check_all_vamps_intermediate.log. If output for any table is greater then 0 - investigate (if renamed, check in the opposite direction and compare resultes.) 
# E.g. SELECT * FROM new_contact_intermediate a LEFT JOIN new_contact b USING(contact) WHERE b.contact_id IS NULL;
# This script does not check vamps sequences - too slow.
# ASh Oct 3 2015
...
echo "SELECT count(*) FROM new_project_dataset_intermediate a RIGHT JOIN new_project_dataset b USING(project_dataset) WHERE a.project_dataset_id IS NULL;"
...
echo "SELECT count(*) FROM new_taxonomy_intermediate a RIGHT JOIN new_taxonomy b USING(taxon_string_id,superkingdom_id,phylum_id,class_id,orderx_id,family_id,genus_id,species_id,strain_id,rank_id,classifier) WHERE a.taxonomy_id IS NULL;"
time mysql -nvv -h vampsdb vamps -e 'SELECT count(*) FROM new_taxonomy_intermediate a RIGHT JOIN new_taxonomy b USING(taxon_string_id,superkingdom_id,phylum_id,class_id,orderx_id,family_id,genus_id,species_id,strain_id,rank_id,classifier) WHERE a.taxonomy_id IS NULL;' >> check_all_vamps_intermediate.log 2>&1

echo "SELECT count(*) FROM new_user_contact_intermediate a RIGHT JOIN new_user_contact b USING(contact_id,user_id) WHERE a.user_contact_id IS NULL;"
time mysql -nvv -h vampsdb vamps -e 'SELECT count(*) FROM new_user_contact_intermediate a RIGHT JOIN new_user_contact b USING(contact_id,user_id) WHERE a.user_contact_id IS NULL;' >> check_all_vamps_intermediate.log 2>&1
...
echo "SELECT count(*) FROM vamps_projects_datasets_intermediate a RIGHT JOIN vamps_projects_datasets b USING(project,dataset) WHERE a.id IS NULL;"
