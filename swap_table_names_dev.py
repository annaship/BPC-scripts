...
import vamps_upload_util_class_no_exp as util
import sys
import collections
...
# shared.my_conn = util.MyConnection(server_name = 'bpcdb2')
# shared.my_conn = util.MyConnection(server_name = 'vampsdb')
# shared.my_conn = util.MyConnection('vampsdev', 'test')
# shared.my_conn = util.MyConnection('bpcdb1', 'env454')
# shared.my_conn = util.MyConnection('vampsdb.mbl.edu', 'vamps')
# shared.my_conn = util.MyConnection('vampsdev', 'vamps2')
...
if __name__ == '__main__':
  ...
  vamps_tables = ["vamps_projects_datasets", "vamps_data_cube", "vamps_junk_data_cube", "vamps_taxonomy", "vamps_sequences", "vamps_projects_info"]
  all_tables   = norm_tables + vamps_tables
  
  intermediate_names = {}
  intermediate_names.update(util.SqlUtil().add_suffix("_intermediate", all_tables))
  transfer_names = {}
  transfer_names.update(util.SqlUtil().add_suffix("_transfer", all_tables))
  previous_names = {}
  previous_names.update(util.SqlUtil().add_suffix("_previous", all_tables))

  print sys.argv[1]
  if (sys.argv[1] == "drop_interm_and_transfer"):
    util.SqlUtil().drop_tables(all_tables, "_intermediate")
    util.SqlUtil().drop_tables(all_tables, "_transfer")

  if (sys.argv[1] == "454"):
    print "2) Drop _intermediate. Dangerous, because foreign check = 0"
    util.SqlUtil().drop_tables(all_tables, "_intermediate")
    print "3) change 454 transfer to intermediate"  
    util.SqlUtil().swap_vamps_tables("_transfer", "_intermediate", all_tables)
  
  if (sys.argv[1] == "ill"):
    print "4) add illumina transfer to intermediate"
    util.SqlUtil().update_intermediate_from_illumina_transfer(intermediate_names, transfer_names)
  
  if (sys.argv[1] == "rename"):  
    print "5) drop previous"
    util.SqlUtil().drop_tables(all_tables, "_previous")
  
    print "6) swap current to previous"
    util.SqlUtil().swap_vamps_tables("", "_previous", all_tables)
      
    print "7) swap intermediate to current"
    util.SqlUtil().swap_vamps_tables("_intermediate", "", all_tables)
